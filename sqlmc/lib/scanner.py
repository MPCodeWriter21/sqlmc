...
class Scanner(Checker):
    ...
    async def scan_single_link(self, href, depth):
        ...
        logger.info(
            f"[{datetime.now()}] Scanned: {href}, Vulnerable: {vulnerable}, Database: {db}"
        )
        self.target_urls.append(
            {
                ...
                self.depth - depth,
                'vulnerable':
                (Color.GREEN + 'Vulnerable' + Color.RESET) if vulnerable else
                (Color.RED + 'Not Vulnerable' + Color.RESET),
